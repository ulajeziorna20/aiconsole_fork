...
@router.get("/initial_directory")
async def initial_directory(project_directory: ProjectDirectory = Depends(project_directory)):
    initial_directory = await project_directory.get_initial_directory()
    return {"directory": None if initial_directory is None else str(initial_directory)}


@router.post("/in_directory")
async def is_project_in_directory(
    params: ProjectDirectoryParams, project_directory: ProjectDirectory = Depends(project_directory)
):
    return {"is_project": project_directory.is_project_in_directory(params.directory)}
