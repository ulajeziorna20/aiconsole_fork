...
async def _handle_acquire_lock_ws_message(connection: AICConnection, message: AcquireLockClientMessage):
    try:
        ...
        await connection.send(ResponseServerMessage(request_id=message.request_id, payload={}, is_error=False))
    except Exception:
        await connection.send(
            ResponseServerMessage(
                request_id=message.request_id, payload={"error": "Error during acquiring lock"}, is_error=True
            )
        )
...
async def _handle_open_chat_ws_message(connection: AICConnection, message: OpenChatClientMessage):
    ...
    try:
        ...
        await connection.send(
            ResponseServerMessage(
                request_id=message.request_id,
                payload={"error": "Error during opening chat"},
                is_error=True,
            )
        )
    finally:
...
async def _handle_close_chat_ws_message(connection: AICConnection, message: CloseChatClientMessage):
    try:
        ...
        await connection.send(
            ResponseServerMessage(
                request_id=message.request_id, payload={"error": "Error during closing chat"}, is_error=True
            )
        )
